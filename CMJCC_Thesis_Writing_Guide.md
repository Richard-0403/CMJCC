...
SC-E-02 与 SC-E-04 的 no-match **不是**硬约束联合不可满足的结果。正确表述是:
**硬约束本身可满足，但满足者不在目标角色范围；同时应用目标角色范围和硬约束后
不存在合格且相关职位。**

依据是数据质量报告的 2 条 `no_match_scenario_constraint_satisfiable` 警示,它们
恰恰说明约束**是**可满足的:

| 场景 | 满足硬约束的 job | 位置 |
|---|---|---|
| `SC-E-02` | 5 个:job-0021、job-0086、job-0089、job-0094、job-0169 | 全部在请求的角色族之外 |
| `SC-E-04` | 1 个:job-0012 | 同上 |

这 2 条警示是**有意保留**的已知张力,不是缺陷 —— 但不能把它们读成"联合不可行"的证据,
它们是相反方向的证据。

两个场景的类型标签是 `multiple_hard` 而非 `no_match`;只有 3 个场景是 `no_match` 类型,
这就是报告的场景类型计数写 `no_match 3`、而 no-match 指标分母是 5 的原因。
`no_match_metrics.csv` 中 precision / recall / F1 = 1.000 的算术成立,但**不能**据此
把 5 个场景整体概括为硬约束联合不可满足。引用冻结报告的
"Correct no-match (SC-E-02)" 案例时必须带上本节限定。

正式勘误见 `final_release/ERRATA.md` E-2 与 `THESIS_OFFICIAL_RESULTS.md` §5.1。
...
- ❌ 把 SC-E-02 / SC-E-04 的 no-match 说成硬约束联合不可满足 —— 约束可满足,
  满足者不在目标角色范围（见 §6）
